...
def _create_plan(session):
    plan = create_training_plan(session, "P", [_rep_plan_input("Bench", 80.0), _rep_plan_input("Row", 70.0)])
...
def test_add_workout_exercise_without_plan_disallows_plan_exercise_reference(session) -> None:
    ...
    w = add_workout_exercise(
        ...
        WorkoutExerciseInput(
            ...
            sets=[WorkoutSetInput(weight=80.0, reps=8, duration_time=None, is_warmup=True)],
        ),
    )

    assert len(w.workout_exercises) == 1
    ex = w.workout_exercises[0]
    assert ex.order_index == 1
...
def test_add_workout_exercise_with_plan_requires_plan_exercise_id_and_belongs_to_plan(session) -> None:
    ...
    w = add_workout_exercise(
        ...
        WorkoutExerciseInput(
            name="Bench",
            plan_exercise_id=bench_plan_id,
            sets=[
...
def test_update_and_remove_workout_exercise_normalizes_order(session) -> None:
    plan = _create_plan(session)
    w = create_workout(session, name="W", training_plan_id=plan.id, started_at=None)
    ...
    w = add_workout_exercise(
        ...
        WorkoutExerciseInput(
            name="Row",
            plan_exercise_id=row_id,
            sets=[WorkoutSetInput(weight=70.0, reps=10, duration_time=None, is_warmup=False)],
        ),
    )
    assert [ex.order_index for ex in w.workout_exercises] == [1, 2]
...
def test_save_workout_as_training_plan(session) -> None:
    ...
    w = add_workout_exercise(
        ...
        WorkoutExerciseInput(
            name="Plank",
            plan_exercise_id=None,
            sets=[WorkoutSetInput(weight=None, reps=None, duration_time=60.0, is_warmup=False)],
        ),
    )
...
def test_save_workout_as_training_plan_raises_if_only_warmups(session) -> None:
    ...
    w = add_workout_exercise(
        ...
        WorkoutExerciseInput(
            name="Bench",
            plan_exercise_id=None,
            sets=[WorkoutSetInput(weight=60.0, reps=5, duration_time=None, is_warmup=True)],
        ),
    )
